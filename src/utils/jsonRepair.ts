  ...
  try {
    ...
    if (actionsBlockMatch) {
      ...
      while ((objMatch = objectPattern.exec(actionsBlock)) !== null) {
        ...
        const actionMatch = objStr.match(/"(?:action|text|title|name|option)"\s*:\s*"([\s\S]*?)"/i);
        const detailsMatch = objStr.match(/"(?:details|description)"\s*:\s*"([\s\S]*?)"/i);
        const timeCostMatch = objStr.match(/"timeCost"\s*:\s*"([\s\S]*?)"/i);
...
export function robustParseGameplayJSON(rawT: string): { parsedData: GameplayParsedData | null; isFallback: boolean } {
  ...
  try {
    ...
    const errorMsg = `[Robust JSON Parser] Parse tiêu chuẩn lỗi, triển khai Phòng tuyến 3 (Regex)...\n  ${err.message || err}`;
    console.warn(errorMsg, err);
    useStore.getState().setSystemLogs((prev) => prev ? prev + "\n" + errorMsg : errorMsg);
